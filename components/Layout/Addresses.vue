<template>
    <div>
            ...
            <div class="flex flex-column gap15 w-100">
                ...
                <label class="radio-bar mb-0" v-for="item in user_addresses" :key="item">
                    <div class="flex align-items-center gap15">
                        <FormRadio name="address" :model="form" :isChecked="item.selected === true ? true : false"
                            :option="item.id" @change="selectAddress(item.id)" />
        ...
        <ClientOnly>
            <Dialog class="site-modal" v-model:visible="address_modal" @hide="resetForm" modal :style="{ width: '615px' }"
                ...
                <div class="container">
                    <form @submit.prevent="map_state === 'edit' ? edit_address() : add_address()" ref="address_form"
                        ...
                        <div class="w-100" v-if="address_modal">
                            ...
                            <FormInput :placeholder="$t('form_layout.select_location')" :readonly="true" :model="form"
                                name="map_desc" type="text" parentClass="my-3 w-100">
                            </FormInput>
                            <GoogleMap @change-address="handellAddress($event)" height="200px" autocomplete
                                apiKey="AIzaSyBNLoYGrbnQI_GMqHt6m0PSN9yA7Zvq7gA" language="ar" :current-address="map_state === 'edit' ? {
                                    geometry: {
