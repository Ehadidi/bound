<template>
    <form @submit.prevent="handelSignUp" ref="signUp_form">
        <div class="grid w-100">
            <FormInput :label="$t('form_layout.user_name')" InputClass="validated" @change="change_valid"
                :placeholder="$t('form_layout.enter_user_name')" :model="form" name="name" type="text"
                parentClass="col-12 lg:col-6" :icon="true">
                <template #icon>
            ...
            <FormInput :label="$t('form_layout.phone')" InputClass="validated" @change="change_valid"
                ...
                parentClass="col-12 lg:col-6" :icon="true" :addition="true">
                <template #icon>
            ...
            <FormInput InputClass="validated" @change="change_valid" :label="$t('form_layout.email')"
                :placeholder="$t('form_layout.enter_email')" :model="form" name="email" type="email" parentClass="col-12"
                :icon="true">
                <template #icon>
            ...
            <div class="col-12">
                ...
                <div class="form-inputs">
                    <Dropdown filter @change="city_check" v-model="selectedCity"
                        ...
                        :emptyMessage="$t('validate_msg.emptyFilterMessage')" :options="cities" optionLabel="name"
                        :placeholder="$t('form_layout.ChooseCity')" class="w-full" />
            ...
            <FormInput InputClass="validated" @change="change_valid" :label="$t('form_layout.password')"
                ...
                type="password" parentClass="col-12" :icon="true">
                <template #icon>
                    <img class="width20" src="~/assets/images/Lock.svg" alt="Lock password">
                </template>
            </FormInput>
            <div class="col-12">
                <div class="flex align-items-center gap5 font13">
                    <FormCheckbox InputClass="check_valid" @change="change_valid($event)" name="is_terms" :model="form" />
                    <span>{{ $t('layout.i_agree') }}</span>
                    <button type="button" class="btn underline p-0 h-fit min-w-min w-fit"
                        @click="terms_conditions = true">{{
                            $t('layout.terms') }}</button>
                </div>
                <Dialog class="site-modal" v-model:visible="terms_conditions" modal :style="{ width: '50vw' }"
                    :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
                    <div class="container">
                        <div class="flex justify-content-end">
                            <button class="btn btn-primary" @click="terms_conditions = false">{{ $t('layout.IAgree')
                            }}</button>
                        </div>
                        <h5 class="fw-bold mb-3 text-primary">{{ $t('layout.terms') }}</h5>
                        <p>
                            Lorem ipsum dolor sit amet consectetur. Vestibulum odio eu duis eget congue semper. Egestas in
                            eget
                            lectus sed consectetur lacus facilisis. Blandit eu in adipiscing at amet. Laoreet purus dui nibh
                            laoreet
                            commodo adipiscing quam lobortis consectetur. Lorem ipsum dolor sit amet consectetur. Vestibulum
                            odio eu
                            duis eget congue semper. Egestas in eget lectus sed consectetur lacus facilisis. Blandit eu in
                            adipiscing at amet. Laoreet purus dui nibh laoreet commodo adipiscing quam lobortis consectetur.
                        </p>
                        <p>
                            Lorem ipsum dolor sit amet consectetur. Vestibulum odio eu duis eget congue semper. Egestas in
                            eget
