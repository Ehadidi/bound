<template>
  <section class="bg-lightGrayClr py-5">
    <div class="container">
      <div class="grig">
        <div
          class="col-12 md:col-8 lg:col-7 mx-auto bg-white position-relative main_shadow"
        >
          <div>
            <button
              @click="address_modal = true"
              class="btn btn-outline-primary up height40 fw-bold"
              style="position: absolute; right: 10px; top: 10px"
            >
              {{ $t("form_layout.add_new_address") }}
            </button>
            <h6 class="fw-bold text-center py-4 border-bottom">
              {{ $t("layout.my_addresses") }}
            </h6>
            <div
              class="flex align-items-center justify-content-center flex-column gap20 p-5"
            >
              <img
                class="w-8rem h-auto"
              ...
              <div class="flex flex-column gap15 w-100">
                ...
                <label
                  class="radio-bar mb-0"
                  v-for="item in user_addresses"
                  :key="item"
                >
                  <div class="flex align-items-center gap15">
                    <FormRadio
                      name="address"
                      :model="form"
                      :isChecked="item.selected === true ? true : false"
                      :option="item.id"
                  ...
                  <div class="flex align-items-center gap15">
                    <button
                      class="btn p-0 border-0 min-w-min h-fit bg-transparent"
                      @click="
                        editAddress(
                          item.name,
                          item.map_desc,
                    ...
                    >
                      <img
                        src="~/assets/images/Pen-edit.svg"
                        alt="edit location"
                      />
                    </button>
                    <button
            ...
            <ClientOnly>
              ...
              >
                ...
                <div class="container">
                  <form
                    @submit.prevent="
                      map_state === 'edit' ? edit_address() : add_address()
                    "
                    ref="address_form"
                  ...
                  >
                    ...
                    <FormInput
                      InputClass="validated height100"
                      :label="$t('form_layout.address_description')"
                      :placeholder="$t('form_layout.enter_address_description')"
                      :model="form"
                      name="description"
                      parentClass="my-3 w-100"
                      :textarea="true"
                    ...
                    <div class="w-100" v-if="address_modal">
                      ...
                      <FormInput
                        :placeholder="$t('form_layout.select_location')"
                        :readonly="true"
                        :model="form"
                        name="map_desc"
                        type="text"
                    ...
                    <button
                      class="btn w-100 btn-primary mb-4 mt-3 up"
                      :loading="loading"
...
const edit_address = async () => {
  ...
  const res = await axios.post(
    `update-address/${address_id.value}?_method=put`,
    fd,
    config
  );
  let status = response(res).status;
