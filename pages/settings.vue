<template>
    <section class="bg-lightGrayClr py-5">
                    ...
                    <form @submit.prevent="edit_profile" ref="edit_form" style="max-width: 90%" class="mx-auto py-3">
                        ...
                        <div class="grid">
                            <div class="col-12 md:col-6">
                                <button type="button" class="btn btn-outline-primary up w-100"
                                    @click="check_password = true">{{
                                        $t('form_layout.change_phone') }}</button>
                            </div>
                            <div class="col-12 md:col-6">
                                <button type="button" class="btn btn-outline-primary up w-100"
                                    @click="change_password_modal = true">{{
                                        $t('form_layout.change_password') }}</button>
                            </div>
                            <div class="col-12">
                                <button type="button" class="btn text-danger fw-bold w-100 flex gap3 align-items-center"
        ...
        <Dialog class="site-modal" v-model:visible="check_password" modal :style="{ width: '615px' }"
            :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <template #header>
                <div class="border-bottom w-100 pt-2 pb-3">
                    <h6 class="text-center fw-bold text-primary">{{ $t('form_layout.change_phone') }}</h6>
                </div>
            </template>
            <div class="container">
                <form @submit.prevent="comfirm_password" class="flex flex-column align-items-center w-75 mx-auto">
                    <div class="my-4 text-center font13">{{ $t('layout.check_password_caption') }}</div>
                    <img class="w-6rem h-auto my-4" src="~/assets/images/phoneLock.png" alt="phone active">

                    <FormInput :label="$t('form_layout.password')" :placeholder="$t('form_layout.enter_password')"
                        :model="form" name="password" type="password" parentClass="my-3 w-100" :icon="true">
                        <template #icon>
        ...
        <Dialog class="site-modal" v-model:visible="check_phone" modal :style="{ width: '615px' }"
            :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <template #header>
                <div class="border-bottom w-100 pt-2 pb-3">
                    <h6 class="text-center fw-bold text-primary">{{ $t('form_layout.change_phone') }}</h6>
            ...
            <div class="container">
                <form @submit.prevent="check_phone_send_code" class="flex flex-column align-items-center w-75 mx-auto"
                    ref="check_phone_form">
                    <div class="my-4 text-center font13">{{ $t('layout.enter_phone_caption') }}</div>
                    <FormInput :label="$t('form_layout.phone')" :placeholder="$t('form_layout.enter_phone')" :model="form"
                        name="phone" type="number" parentClass="w-100" :icon="true" :addition="true">
                        <template #icon>
        ...
        <Dialog class="site-modal" v-model:visible="change_avtivation" modal :style="{ width: '615px' }"
            :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <template #header>
                <div class="border-bottom w-100 pt-2 pb-3">
                    <h6 class="text-center fw-bold text-primary">{{ $t('form_layout.change_phone') }}</h6>
            ...
            <div class="container">
                <form @submit.prevent="verification" class="flex flex-column align-items-center">
                    ...
                    <div class="mt-5 mx-auto flex flex-column align-items-center">
                        <v-otp-input dir="ltr" class="otp-inputs" ref="otpInput" v-model:value="bindModal"
                            input-classes="otp-input" separator="   " :num-inputs="4" :should-auto-focus="true"
                            input-type="letter-numeric" :conditionalClass="['one', 'two', 'three', 'four']"
                        ...
                        <div class="flex align-items-center justify-content-between w-100">
                            <button type="button" class="btn p-0 underline fw-bold font13 w-fit min-w-min"
                                @click="resend_code" disabled>
                                <span>{{ $t('form_layout.reSend') }}</span>
                                <div class="spinner-border spinner-border-sm d-none" :class="{ 'd-block': LoadBtn }"
                                    role="status">
                                    <span class="sr-only">Loading...</span>
                                </div>
                            </button>
        ...
        <Dialog class="site-modal" v-model:visible="change_password_modal" modal :style="{ width: '615px' }"
            :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <template #header>
                <div class="border-bottom w-100 pt-2 pb-3">
                    <h6 class="text-center fw-bold text-primary">{{ $t('form_layout.change_phone') }}</h6>
            ...
            <div class="container">
                <form @submit.prevent="change_password" class="flex flex-column align-items-center w-75 mx-auto"
                    ref="change_password_fornm">
                    <div class="my-4 text-center font13">{{ $t('layout.check_password_caption') }}</div>
                    <img class="w-6rem h-auto my-4" src="~/assets/images/phoneLock.png" alt="phone active">

                    <FormInput InputClass="validated" @change="change_valid" :label="$t('form_layout.current_password')"
                        :placeholder="$t('form_layout.enter_current_password')" :model="form" name="old_password"
                        type="password" parentClass="w-100" :icon="true">
                        <template #icon>
                            <img class="width20" src="~/assets/images/Lock.svg" alt="Lock password">
                        </template>
                    </FormInput>
                    <FormInput InputClass="validated" @change="change_valid" :label="$t('form_layout.new_password')"
                        :placeholder="$t('form_layout.enter_new_password')" :model="form" name="password" type="password"
                        parentClass="w-100" :icon="true">
                        <template #icon>
                            <img class="width20" src="~/assets/images/Lock.svg" alt="Lock password">
                        </template>
                    </FormInput>
                    <FormInput InputClass="validated" @change="change_valid" :label="$t('form_layout.new_password')"
                        :placeholder="$t('form_layout.re_enter_new_password')" :model="form" name="password_confirmation"
                        type="password" parentClass="w-100" :icon="true">
                        <template #icon>
                            <img class="width20" src="~/assets/images/Lock.svg" alt="Lock password">
                        </template>
...
const change_password = async () => {
    ...
    let valid = validate(change_password_fornm.value).valid;
