...
export function validate(form) {
    if (form) {
        ...
        if (radioGroups) {
            for (let G = 0; G < radioGroups.length; G++) {
                ...
                for (let R = 0; R < InputsRadio.length; R++) {
                    ...
                    let radio_feed = input_radio.closest(".radio_group").getElementsByClassName('feedback')[0]
                    if (input_radio.checked) {
                        ...
                        return_valid(input_radio.name, radio_feed, '')
                    }
                }
        ...
        if (checkGroups) {
            for (let G = 0; G < checkGroups.length; G++) {
                ...
                for (let R = 0; R < InputsCheck.length; R++) {
                    ...
                    let check_feed = input_check.closest(".check_group").getElementsByClassName('feedback')[0]
                    if (!check_feed) {
                    ...
                    if (input_check.checked) {
                        ...
                        return_valid(input_check.name, check_feed, '')
                    }
                }
        ...
        for (let i = 0; i < FormArray.length; i++) {
            ...
            let feedback = Input.parentElement.parentElement.getElementsByClassName('feedback')[0]
            if (!feedback) {
            ...
            if (value == '') {
                if (Input.parentElement.classList.contains('p-calendar')) {
                    let FeedBack = Input.closest(".form__label").getElementsByClassName('feedback')[0]
                    let InputName = Input.closest(".p-calendar").getAttribute("name")
                    ErrorValids.push(InputName)
                    return_valid(InputName, FeedBack, '')
                } else {
                    ErrorValids.push(Input.name)
                    return_valid(Input.name, feedback, '')
                }
            } else if (value != '') {
                //============== email check ==============>
                if (type == 'email') {
...
export function change_valid(e) {
    let Input = e.target
    let type = Input.type
    let value = Input.value
    ...
    //============== radio check ==============>
        ...
        let radio_feedback = Input.closest(".radio_group").getElementsByClassName('feedback')[0]
        if (Input.checked) {
            empty_valid(radio_feedback)
        } else {
            return_valid(InputName, radio_feedback, '')
        }
    }
    //============== checkbox check ==============>
        ...
        let check_feedback = Input.closest(".check_group").getElementsByClassName('feedback')[0]
        for (let C = 0; C < seblig_check.length; C++) {
            ...
            if (checked.length) {
                ...
                return_valid(InputName, check_feedback, '')
        ...
        if (value === '') {
            ...
            if (type == 'email') {
                if (/^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/.test(value)) {
                    ...
                    return_valid(InputName, feedback, "_wrong")
                }
            }
            //============== phone check ==============>
